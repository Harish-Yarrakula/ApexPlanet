       // Performance optimization: Use efficient data structures
        class ShoppingApp {
            ...
            generateProducts() {
                const productTypes = [
                    ...
                    { name: 'Keyboard', icon: '⌨️', basePrice: 89 },
                    { name: 'Mouse', icon: '🖱️', basePrice: 59 },
                    { name: 'Monitor', icon: '🖥️', basePrice: 329 }
                ];
        ...
        const improveAccessibility = () => {
            ...
            cartButton.setAttribute('aria-describedby', 'cart-description');
            
            const cartDescription = document.createElement('span');
            cartDescription.id = 'cart-description';
            cartDescription.className = 'sr-only';
            cartDescription.textContent = 'Shopping cart with ' + document.getElementById('cartCount').textContent + ' items';
            document.body.appendChild(cartDescription);
        };

        improveAccessibility();

        // Progressive enhancement
        const enhanceForModernBrowsers = () => {
            // Add advanced animations for modern browsers
            if (CSS.supports('animation-timeline', 'scroll()')) {
                // Advanced scroll-triggered animations
                document.body.classList.add('modern-animations');
            }

            // Add advanced grid features for supporting browsers
            if (CSS.supports('display', 'subgrid')) {
                document.body.classList.add('subgrid-support');
            }
        };

        enhanceForModernBrowsers();

        console.log('TechStore E-commerce App loaded successfully!');
        console.log('Features included: Responsive design, Performance optimization, Cross-browser compatibility, Accessibility support');
